    ...
    fn fmt(&self, f: &mut Formatter<'_>) -> std::fmt::Result {
        match self {
            ...
            Self::Api {
                ...
                (Some(error_type), Some(message)) if !message.is_empty() && message != "Error" => {
                    write!(f, "api returned {status} ({error_type}): {message}")
                }
                (Some(error_type), _) if status.as_u16() == 429 => {
                    write!(f, "api returned {status} ({error_type}): credentials may be expired — run `claw-tui login` to refresh")
                }
