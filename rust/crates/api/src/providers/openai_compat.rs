...
const GITHUB_COPILOT_ENV_VARS: &[&str] = &["GITHUB_COPILOT_TOKEN"];
...
impl OpenAiCompatConfig {
    ...
    pub const fn github_copilot() -> Self {
        Self {
            ...
            default_base_url: DEFAULT_COPILOT_BASE_URL,
        }
    }
...
pub struct ExtraHeaders(pub Vec<(&'static str, String)>);
...
impl OpenAiCompatClient {
    ...
    /// Create a client reusing an existing `reqwest::Client` (avoids recreating
    /// the connection pool for providers that manage their own HTTP client).
    #[must_use]
    pub fn from_client(
        http: reqwest::Client,
