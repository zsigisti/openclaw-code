...
impl ClawApiClient {
    ...
    /// Exchange an OAuth access token for a long-lived Anthropic workspace API key
    /// (`sk-ant-api03-…`) via the Claude CLI key-creation endpoint.
    pub async fn create_api_key_from_oauth(
        ...
        let key = body
            ...
            .ok_or_else(|| ApiError::Auth(format!(
                "unexpected create_api_key response: {body}"
            )))?
            .to_string();
        Ok(key)
    }
    ...
    async fn send_raw_request(
        ...
        // Enable OAuth bearer token support when a bearer token is present.
