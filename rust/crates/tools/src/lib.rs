...
fn run_bash(input: BashCommandInput) -> Result<String, String> {
    ...
    let out = execute_bash(input).map_err(|e| e.to_string())?;

    // Format like Claude Code: Bash(command)\n⎿  output
    let mut lines: Vec<&str> = Vec::new();
    if !out.stdout.is_empty() {
        lines.extend(out.stdout.trim_end().lines());
    }
    if !out.stderr.is_empty() {
        lines.extend(out.stderr.trim_end().lines());
    }

    let output_block = if lines.is_empty() {
        String::new()
    } else {
        lines
            .iter()
            .enumerate()
            .map(|(i, l)| if i == 0 { format!("  {l}") } else { format!("\n  {l}") })
            .collect::<String>()
    };
...
fn run_write_file(input: WriteFileInput) -> Result<String, String> {
    let out = write_file(&input.path, &input.content).map_err(io_to_string)?;
    Ok(format_write_diff(&out))
}
...
fn run_edit_file(input: EditFileInput) -> Result<String, String> {
    ...
    Ok(format_edit_diff(&out))
}

fn format_edit_diff(out: &runtime::EditFileOutput) -> String {
    // Shorten the path to be relative-looking if it contains common path segments.
    let path = shorten_path(&out.file_path);
    let removed = out.old_string.lines().count();
    let added = out.new_string.lines().count();
    let summary = diff_summary(removed, added);

    let mut buf = format!("Update({path})\n\u{23bf}  {summary}\n");

    // Show context lines (3 before old_string) + removed/added lines.
    let ctx_before = context_before(&out.original_file, &out.old_string, 3);
    for line in &ctx_before {
        buf.push_str(&format!("  {line}\n"));
    }
    for line in out.old_string.lines() {
        buf.push_str(&format!("-{line}\n"));
    }
    for line in out.new_string.lines() {
        buf.push_str(&format!("+{line}\n"));
    }
    // 3 lines of context after the replaced region.
    for line in context_after(&out.original_file, &out.old_string, 3) {
        buf.push_str(&format!("  {line}\n"));
    }
    buf
}

fn format_write_diff(out: &runtime::WriteFileOutput) -> String {
    let path = shorten_path(&out.file_path);
    let is_create = out.kind == "create";
    let verb = if is_create { "Create" } else { "Update" };
    let old_lines = out.original_file.as_deref().unwrap_or("").lines().count();
    let new_lines = out.content.lines().count();
    let summary = if is_create {
        format!("Added {new_lines} lines")
    } else {
        diff_summary(old_lines, new_lines)
    };

    let mut buf = format!("{verb}({path})\n\u{23bf}  {summary}\n");

    // For large files just show the first 30 diff lines.
    const MAX_DIFF_LINES: usize = 30;
    let old: Vec<&str> = out.original_file.as_deref().unwrap_or("").lines().collect();
    let new: Vec<&str> = out.content.lines().collect();
    let mut shown = 0;
    for line in &old {
        if shown >= MAX_DIFF_LINES { buf.push_str("  …\n"); break; }
        buf.push_str(&format!("-{line}\n"));
        shown += 1;
    }
    for line in &new {
        if shown >= MAX_DIFF_LINES * 2 { buf.push_str("  …\n"); break; }
        buf.push_str(&format!("+{line}\n"));
        shown += 1;
    }
    buf
...
fn diff_summary(removed: usize, added: usize) -> String {
    match (removed, added) {
        (0, a) => format!("Added {a} lines"),
        (r, 0) => format!("Removed {r} lines"),
        (1, 1) => "Changed 1 line".to_string(),
        (r, a) => format!("Removed {r} lines, added {a} lines"),
    }
}

fn shorten_path(path: &str) -> String {
    // Prefer a path relative to CWD.
    ...
    // Strip everything up to and including the workspace root heuristic.
    for marker in &["crates/", "src/"] {
        if let Some(pos) = path.find(marker) {
            return path[pos..].to_string();
        }
    }
    // Fall back to the full path as-is.
    path.to_string()
}

fn context_before<'a>(content: &'a str, target: &str, n: usize) -> Vec<&'a str> {
    let pos = match content.find(target) {
        Some(p) => p,
        None => return Vec::new(),
    };
...
fn context_after<'a>(content: &'a str, target: &str, n: usize) -> Vec<&'a str> {
    let pos = match content.find(target) {
        Some(p) => p,
        None => return Vec::new(),
    };
    ...
    let after = &content[after_pos..];
    after.lines().take(n).collect()
}
